...
type ReportCall = { id: string; status?: string; finalDisposition?: string; createdAt?: string; campaign?: EntitySummary; operator?: EntitySummary; lead?: LeadSummary };
...
export default function ReportsPro() {
  ...
  function clearFilters() { setCampaign(''); setOperator(''); setStatus(''); setDateFrom(''); setDateTo(''); setPage(1); }
  function download() { exportCsv('relatorio-atendimentos-referencia.csv', ['campanha', 'operador', 'lead', 'telefone', 'status_chamada', 'desfecho', 'status_lead', 'data'], filteredCalls.map((row) => [row.campaign?.name, row.operator?.name, row.lead?.name, row.lead?.phoneNormalized, row.status, row.finalDisposition, row.lead?.status, row.createdAt])); }
  ...
  return <section className="reportsPro polishPage">
    ...
    <div className="panel filtersPanel"><div className="panelHeader"><h3><Search size={20} />Filtros de gestão</h3><button className="ghostBtn" onClick={clearFilters}>Limpar filtros</button></div><div className="filters reportFilters polishFilters reportFiltersExtended"><label>Campanha<select value={campaign} onChange={(event) => setCampaign(event.target.value)}><option value="">Todas</option>{campaigns.map((value) => <option key={value}>{value}</option>)}</select></label><label>Operador<select value={operator} onChange={(event) => setOperator(event.target.value)}><option value="">Todos</option>{operators.map((value) => <option key={value}>{value}</option>)}</select></label><label>Status/desfecho<select value={status} onChange={(event) => setStatus(event.target.value)}><option value="">Todos</option>{statuses.map((value) => <option key={value}>{formatStatus(value)}</option>)}</select></label><label>De<input type="date" value={dateFrom} max={dateTo || undefined} onChange={(event) => setDateFrom(event.target.value)} /></label><label>Até<input type="date" value={dateTo} min={dateFrom || undefined} onChange={(event) => setDateTo(event.target.value)} /></label><button onClick={() => void load()} disabled={loading}><RefreshCw size={16} />{loading ? 'Buscando...' : 'Buscar'}</button></div></div>
