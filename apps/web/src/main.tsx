...
function Login({ on }: { on: () => void }) {
  ...
  return <main className="loginPage"><section className="loginHero"><div className="brandMark">IA</div><h1>{APP_NAME}</h1><p>Central de campanhas, listas, fila de ligação, telefone SIP/WebRTC, desfechos, produtividade e compliance para operação ativa.</p><div className="heroBullets"><span>Campanhas</span><span>Discagem</span><span>Softphone SIP</span><span>LGPD</span></div></section><form onSubmit={submit} className="loginCard"><small>Acesso restrito</small><h2>Entrar na operação</h2><label>E-mail<input value={email} onChange={(e) => setEmail(e.target.value)} /></label><label>Senha<input type="password" value={password} onChange={(e) => setPassword(e.target.value)} /></label><Notice msg={msg} /><button disabled={loading}>{loading ? 'Entrando...' : 'Entrar'}</button><p className="muted">Use apenas credenciais autorizadas. Todas as ações relevantes devem ser auditáveis.</p></form></main>;
}
...
function Dashboard({ setPage }: { setPage: (p: string) => void }) {
  ...
  return <><div className="heroPanel"><div><small>Visão geral</small><h2>Operação de discagem com fluxo de ponta a ponta</h2><p>Campanha, fila, discagem, screen pop, telefone SIP/WebRTC, desfecho, bloqueio e relatórios.</p></div><button onClick={reload}><RefreshCw size={17} />Atualizar</button></div><div className="pipeline"><div>Base importada</div><div>Fila ativa</div><div>Softphone SIP</div><div>Desfecho</div><div>Relatório</div></div>{error && <Notice msg={{ type: 'err', text: error }} />}<div className="kpiGrid">{cards.map(([a, b, c]) => <div className="kpi" key={a}><small>{a}</small><strong>{loading ? '...' : fmt(b)}</strong><span>{c}</span></div>)}</div><section className="twoCols"><div className="panel"><h3>Atalhos operacionais</h3><div className="steps"><button onClick={() => setPage('Campanhas')}>1. Criar campanha</button><button onClick={() => setPage('Importar Leads')}>2. Importar leads</button><button onClick={() => setPage('Softphone SIP')}>3. Registrar telefone</button><button onClick={() => setPage('Mesa do Operador')}>4. Operar fila</button></div></div><div className="panel accent"><h3>Status da telefonia</h3><p>O MicroSIP já validou seu Asterisk por UDP. No navegador, a conexão será por WebRTC/WSS; a tela Softphone SIP já está preparada para registrar o ramal quando o WSS estiver ativo.</p></div></section></>;
}
...
function ImportLeads() {
  ...
  async function up(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setMsg({ type: 'ok', text: `Importação: ${d.totalImported} importados, ${d.duplicates} duplicados, ${d.invalid} inválidos, ${d.blocked} bloqueados.` });
...
function Leads() {
  ...
  useEffect(() => { load(); }, []);
  async function updateLead(row: any, status: string) {
    try { await api(`/leads/${row.id}`, { method: 'PATCH', body: JSON.stringify({ status }) }); setMsg({ type: 'ok', text: `Lead marcado como ${status.replaceAll('_', ' ')}` }); load(); } catch (e: any) { setMsg({ type: 'err', text: e.message }); }
  }
...
function Operator({ setPage }: { setPage: (p: string) => void }) {
  ...
  const [msg, setMsg] = useState<Msg>(null);
  async function load() { const r = await api('/leads?status=EM_FILA'); setQueue(r); setLead(r[0]); }
  useEffect(() => { load().catch(() => {}); }, []);
  async function start() { if (!lead) return; try { setCall(await api('/calls/start', { method: 'POST', body: JSON.stringify({ leadId: lead.id }) })); setMsg({ type: 'ok', text: 'Chamada criada no histórico. Use o Softphone SIP para discar de verdade.' }); } catch (e: any) { setMsg({ type: 'err', text: e.message }); } }
  async function finish(s: string) { if (!call) return; try { await api(`/calls/${call.id}/finish`, { method: 'POST', body: JSON.stringify({ finalDisposition: s, notes, doNotCall: s === 'NAO_LIGAR_NOVAMENTE' }) }); setMsg({ type: 'ok', text: `Desfecho salvo: ${s.replaceAll('_', ' ')}` }); setCall(undefined); setNotes(''); load(); } catch (e: any) { setMsg({ type: 'err', text: e.message }); } }
  return <section className="operatorLayout"><div className="panel leadCard"><h3>Screen pop do lead</h3><p className="muted">Dados principais aparecem antes da ligação para reduzir tempo de atendimento.</p><Notice msg={msg} />{lead ? <><div className="person"><small>Lead em fila</small><h2>{lead.name}</h2><p>{lead.phoneNormalized}</p><span>{lead.course || 'Curso não informado'} • {lead.origin || 'Origem não informada'}</span></div><div className="callActions"><button className="callBtn" onClick={() => setPage('Softphone SIP')}><PhoneCall size={20} />Abrir telefone</button><button onClick={start}><ClipboardList size={18} />Criar histórico</button><textarea placeholder="Observação do atendimento" value={notes} onChange={(e) => setNotes(e.target.value)} /></div><div className="dispositions">{dispositions.map((s) => <button key={s} disabled={!call} onClick={() => finish(s)}>{s.replaceAll('_', ' ')}</button>)}</div></> : <Empty text="Nenhum lead em fila." />}</div><div className="panel"><h3>Próximos da fila</h3>{queue.slice(0, 10).map((l, i) => <button className="queueItem" key={l.id} onClick={() => setLead(l)}>{i + 1}. {l.name}<span>{l.phoneNormalized}</span></button>)}</div></section>;
}
...
function Voip() {
  ...
  return <section className="twoCols"><div className="panel"><h3>Configuração VoIP</h3><p className="muted">O Asterisk já foi validado com MicroSIP por UDP. Para o navegador, ative o WebRTC/WSS no Asterisk e use a tela Softphone SIP.</p><DataTable rows={[data || { provider: 'Asterisk', asteriskHost: window.location.hostname, asteriskPort: 5060, asteriskContext: 'interno' }]} columns={[{ key: 'provider', label: 'Provider' }, { key: 'asteriskHost', label: 'Host' }, { key: 'asteriskPort', label: 'Porta' }, { key: 'asteriskContext', label: 'Contexto' }]} /></div><div className="panel accent"><h3>Próximas integrações</h3><div className="miniGuide"><span>WebRTC/WSS para áudio direto no navegador.</span><span>Screen pop em chamada recebida.</span><span>Discagem pelo lead da fila.</span><span>Gravação e auditoria por chamada.</span></div></div></section>;
}

function App() {
  const [ok, setOk] = useState(!!localStorage.getItem('token'));
  const [page, setPage] = useState<string>('Dashboard');
  const content = useMemo(() => page === 'Dashboard' ? <Dashboard setPage={setPage} /> : page === 'Campanhas' ? <Campaigns /> : page === 'Importar Leads' ? <ImportLeads /> : page === 'Leads' ? <Leads /> : page === 'Mesa do Operador' ? <Operator setPage={setPage} /> : page === 'Softphone SIP' ? <SipSoftphone /> : page === 'Relatórios' ? <Reports /> : page === 'Não Ligar' ? <DoNotCall /> : page === 'Usuários' ? <UsersPage /> : <Voip />, [page]);
