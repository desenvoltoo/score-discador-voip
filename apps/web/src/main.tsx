...
import UsersPro from './pages/UsersPro';
import './style.css';
...
function App() {
  ...
  const content = useMemo(() => page === 'Dashboard' ? <Dashboard setPage={setPage} /> : page === 'Central IA' ? <GrowthCommandCenter /> : page === 'Campanhas' ? <Campaigns /> : page === 'Importar Leads' ? <ImportLeadsPro /> : page === 'Hist. Importações' ? <ImportHistoryPage openImport={() => setPage('Importar Leads')} /> : page === 'Leads' ? <LeadsPro openOperator={() => setPage('Mesa do Operador')} /> : page === 'Retornos' ? <ReturnsPage openOperator={() => setPage('Mesa do Operador')} /> : page === 'Mesa do Operador' ? <OperatorDeskPro openPhone={() => setPage('Softphone SIP')} /> : page === 'Softphone SIP' ? <SipSoftphone /> : page === 'Relatórios' ? <ReportsPro /> : page === 'Qualidade' ? <QualityCenterPage openReports={() => setPage('Relatórios')} /> : page === 'Não Ligar' ? <DoNotCallPro /> : page === 'Usuários' ? <UsersPro /> : <Voip />, [page]);
