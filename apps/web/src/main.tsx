...
function Login({ on }: { on: () => void }) {
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault();
    try { setLoading(true); const d = await api('/auth/login', { method: 'POST', body: JSON.stringify({ email, password }) }); localStorage.setItem('token', d.token); on(); }
...
function Dashboard({ setPage }: { setPage: (p: string) => void }) {
  ...
  return <><div className="heroPanel heroPremium"><div><small>Visão geral</small><h2>Operação comercial com telefonia WebRTC e inteligência de atendimento</h2><p>Campanha, fila, chamada no navegador, screen pop, desfecho, LGPD e análise gerencial no mesmo fluxo.</p></div><button onClick={reload}><RefreshCw size={17} />Atualizar</button></div><div className="pipeline"><div>Base importada</div><div>Score de prioridade</div><div>Softphone SIP</div><div>Agent Assist</div><div>Relatório</div></div>{error && <Notice msg={{ type: 'err', text: error }} />}<div className="kpiGrid">{cards.map(([a, b, c]) => <div className="kpi" key={a}><small>{a}</small><strong>{loading ? '...' : fmt(b)}</strong><span>{c}</span></div>)}</div><section className="twoCols"><div className="panel"><h3><Sparkles size={19} />Atalhos de alta performance</h3><div className="steps"><button onClick={() => setPage('Central IA')}>0. Central IA</button><button onClick={() => setPage('Campanhas')}>1. Criar campanha</button><button onClick={() => setPage('Importar Leads')}>2. Importar leads</button><button onClick={() => setPage('Mesa do Operador')}>3. Operar com IA</button></div></div><div className="panel accent"><h3><Activity size={19} />Próximo diferencial</h3><p>O cockpit do operador concentra fila, lead, telefone, desfecho e uma camada visual de IA para acelerar atendimento e padronizar qualidade.</p></div></section></>;
}
...
function Campaigns() {
  ...
  const rowMenu = (r: any) => [{ label: 'Copiar nome', icon: <Copy size={15} />, action: () => navigator.clipboard.writeText(r.name || '') }, { label: 'Atualizar lista', icon: <RefreshCw size={15} />, action: reload }];
...
function App() {
  ...
  const content = useMemo(() => page === 'Dashboard' ? <Dashboard setPage={setPage} /> : page === 'Central IA' ? <GrowthCommandCenter /> : page === 'Campanhas' ? <Campaigns /> : page === 'Importar Leads' ? <ImportLeadsPro /> : page === 'Leads' ? <Leads /> : page === 'Mesa do Operador' ? <Operator setPage={setPage} /> : page === 'Softphone SIP' ? <SipSoftphone /> : page === 'Relatórios' ? <Reports /> : page === 'Não Ligar' ? <DoNotCall /> : page === 'Usuários' ? <UsersPage /> : <Voip />, [page]);
